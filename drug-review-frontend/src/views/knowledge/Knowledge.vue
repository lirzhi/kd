        ...
        <el-card class="knowledge-card">
          <div slot="header" class="card-header">
            <div class="header-left">
              <span class="page-title">{{ currentTypeLabel }}</span>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        classification: [
          { required: true, message: '请输入知识类型', trigger: 'blur' }
        ],
        affect_range: [
  ...
  computed: {
    currentTypeLabel() {
      if (this.activeType === 'all') return '知识总览'
      const type = this.typeList.find(t => t.value === this.activeType)
  ...
  methods: {
      ...
      try {
        ...
        if (response.data.code === 200) {
          this.typeList = response.data.data.map(item => ({
            value: item !== 'eCTD' ? item : null,
            label: item !== 'eCTD' ? item : null,
          })).filter(item => item.value !== null)
        }
      } catch (error) {
        console.error('获取知识类型列表失败:', error)
        this.$message.error('获取知识类型列表失败，请重试')
      }
    },
    async getFileList() {
      this.loading = true
      const classification = this.activeType === 'all' ? 'all' : this.activeType
      try {
        const response = await this.$http.get(`/api/get_file_by_class/${classification}`, {
          params: {
    ...
    handleAdd() {
      this.form = {
        classification: this.activeType === 'all' ? '' : this.activeType,
        affect_range: '',
        file: null
      }
      this.fileList = []
