<template>
    <div class="knowledge-container">
          ...
          <el-card class="knowledge-card">
            <div slot="header" class="card-header">
              <div class="header-left">
                <span class="page-title">问答数据</span>
                <el-tag type="info" class="total-count" v-if="activeTab==='file'">共 {{ total }} 条记录</el-tag>
                <el-tag type="info" class="total-count" v-else>共 {{ qaTotal }} 条记录</el-tag>
              </div>
              <div class="header-right">
                <el-select v-model="activeTab" style="width:180px;margin-right:10px" @change="handleTabChange">
                  <el-option label="问答数据信息" value="info" />
                  <el-option label="问答数据文件" value="file" />
                </el-select>
                <el-button type="primary" icon="el-icon-plus" @click="activeTab==='file'?handleAdd():handleQAAdd()">
                  {{ activeTab==='file' ? '新增文件' : '新增知识' }}
                </el-button>
                <el-button type="primary" icon="el-icon-refresh" @click="activeTab==='file'?handleRefresh():getQAList()">刷新</el-button>
              </div>
            </div>
            <div class="table-scroll-area qa-table-scroll-area" :style="activeTab==='info' ? 'height:calc(100vh - 260px);overflow-y:auto;' : ''">
              ...
              <el-table v-else
                ...
                v-loading="loading"
              >
                <el-table-column prop="category" label="问答属性类型" width="180" align="center"></el-table-column>
                <el-table-column prop="question" label="问题" min-width="300" show-overflow-tooltip></el-table-column>
                <el-table-column prop="answer" label="回答" min-width="300" show-overflow-tooltip></el-table-column>
                <el-table-column label="操作" width="180" fixed="right" align="center">
                  <template slot-scope="scope">
                    <el-button size="mini" type="primary" @click="handleQAEdit(scope.row)">编辑</el-button>
                    <el-button size="mini" type="danger" @click="handleQADelete(scope.row)">删除</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
            ...
            <pagination
              v-else
              :total="qaTotal"
              :page.sync="qaQuery.page"
              :limit.sync="qaQuery.limit"
              @pagination="getQAList"
              class="pagination"
            />
          </el-card>
        </el-main>
      ...
      <!-- 新增/编辑问答知识对话框 -->
      <el-dialog
        :title="qaEditMode ? '编辑知识' : '新增知识'"
        :visible.sync="qaDialogVisible"
        width="50%"
        :close-on-click-modal="false"
        @closed="handleQADialogClosed"
      >
        <el-form
          ref="qaFormRef"
          :model="qaForm"
        ...
        >
          <el-form-item label="问答属性类型" prop="category">
            <el-input v-model="qaForm.category" placeholder="请输入问答属性类型"></el-input>
          </el-form-item>
          <el-form-item label="问题" prop="question">
            <el-input v-model="qaForm.question" placeholder="请输入问题"></el-input>
          </el-form-item>
          <el-form-item label="回答" prop="answer">
            <el-input v-model="qaForm.answer" type="textarea" :rows="4" placeholder="请输入回答"></el-input>
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click="qaDialogVisible = false">取 消</el-button>
          <el-button type="primary" @click="submitQAForm" :loading="qaSubmitting">保 存</el-button>
        </div>
      </el-dialog>
  ...
  export default {
    ...
    data() {
      return {
        ...
        qaRules: {
          category: [{ required: true, message: '请输入问答属性类型', trigger: 'blur' }],
    ...
    created() {
      this.activeTab==='file'?this.getFileList():this.getQAList()
    },
    methods: {
      handleTabChange() {
        if(this.activeTab==='file') this.getFileList();
        else this.getQAList();
      },
      async getFileList() {
        this.loading = true
      ...
      async getQAList() {
        this.loading = true
        try {
          const res = await this.$http.get('/api/qa/list', { params: { page: this.qaQuery.page, limit: this.qaQuery.limit } })
          if(res.data.code===200) {
            this.qaList = res.data.data.list
            this.qaTotal = res.data.data.total
          }
        } catch(e) {
      ...
      handleQAAdd() {
        this.qaEditMode = false
        this.qaForm = { category: '', question: '', answer: '' }
        this.qaDialogVisible = true
        this.$nextTick(()=>{ this.$refs.qaFormRef && this.$refs.qaFormRef.clearValidate() })
      },
      handleQAEdit(row) {
        this.qaEditMode = true
        this.qaForm = { ...row }
        this.qaDialogVisible = true
        this.$nextTick(()=>{ this.$refs.qaFormRef && this.$refs.qaFormRef.clearValidate() })
      },
      handleQADelete(row) {
        this.$confirm('确认删除该问答吗？','提示',{type:'warning'}).then(async()=>{
          const res = await this.$http.post(`/api/qa/delete/${row.id}`)
          if(res.data.code===200) {
            this.$message.success('删除成功')
            this.getQAList()
          } else {
      ...
      async submitQAForm() {
        ...
        this.$refs.qaFormRef.validate(async valid => {
          if(!valid) { this.qaSubmitting = false; return }
          if(this.qaEditMode) {
            // 编辑
            const res = await this.$http.post(`/api/qa/update/${this.qaForm.id}`, this.qaForm)
            if(res.data.code===200) {
            ...
            // 新增
            const res = await this.$http.post('/api/qa/add', this.qaForm)
            if(res.data.code===200) {
